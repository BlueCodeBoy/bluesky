...
import scala.math.min
 ...
 object PostgresDialect extends JdbcDialect {
   override def requiresCastToVariousForIsNull:Boolean={
     true
   }
  ...
  private def toCatalystType(
    ...
    case "numeric" | "decimal" => Some( DecimalType(min(precision, MAX_PRECISION), min(scale, MAX_SCALE)))
    case _ => None
  }
  ...
  override def getJDBCType(dt: DataType): Option[JdbcType] = dt match {
    ...
    case ArrayType(et, _) if et.isInstanceOf[String] =>
