...
case object DeleteObject {
  ...
  /**
    ...
    * @return
    */
  def applyConnection(objectMeta: DeleteObjectMeta)  = {
  ...
  /**
    ...
    * @param dataFrame
    */
  def deleteData(objectMeta: DeleteObjectMeta,dataFrame: DataFrame):Unit = {
    dataFrame.foreachPartition(data=>{
      if(!data.isEmpty){
        val conn = DeleteObject.applyConnection(objectMeta)
        DeleteObject.deleteDatabByPartition(conn,data.toIterator,
          objectMeta)
      }
    })
  }
  def deleteDatabByPartition(
    ...
    try {
      ...
      val stmt = deleteStatement(conn ,deleteObjectMeta )
      try {
        var rowCount = 0
        while (iterator.hasNext) {
          ...
          var j = 0
          var index = 0
          while (j < deleteObjectMeta.keyStream.length){
            index  = row.fieldIndex(deleteObjectMeta.keyStream.apply(j))
            setDataByRow(row,index,j,stmt,deleteObjectMeta.pluginId,conn)
            j+=1
          }
          var k = 0
...
//          }
  ...
  : PreparedStatement = {
    val jdbcDialect =  getJdbcDialect(deleteObjectMeta.pluginId)
    var sql:mutable.StringBuilder = new mutable.StringBuilder(s"delete from  ${deleteObjectMeta.tableName} $CR  ")

    sql.append("WHERE ")

    var j = 0
    while ( {
      j < deleteObjectMeta.keyLookup.length
    }) {
      if (j != 0){
        sql.append("AND   ")
      }
      sql.append(" ( ( ")
      sql.append(jdbcDialect.quoteIdentifier(deleteObjectMeta.keyLookup.apply(j)))
      if ("BETWEEN".equalsIgnoreCase(deleteObjectMeta.keyCondition.apply(j))) {
        sql.append(" BETWEEN ? AND ? ")
      } else if ("IS NULL".equalsIgnoreCase(deleteObjectMeta.keyCondition.apply(j)) || "IS NOT NULL".equalsIgnoreCase(deleteObjectMeta.keyCondition.apply(j))){
        sql .append(s" ${deleteObjectMeta.keyCondition.apply(j)} ")
      }
      else if ("= ~NULL".equalsIgnoreCase(deleteObjectMeta.keyCondition.apply(j))) {
...
//        }
        // null check
        sql.append(s" ) OR (  ${jdbcDialect.quoteIdentifier(deleteObjectMeta.keyLookup.apply(j))} = ?")
