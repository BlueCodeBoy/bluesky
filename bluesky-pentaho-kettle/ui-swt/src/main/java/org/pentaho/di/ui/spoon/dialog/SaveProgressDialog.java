...
public class SaveProgressDialog {
  ...
  public boolean open() {
    ...
    try {
      ...
      //屏蔽保存过程中的取消按钮  2017-11-16 by llj
      pmd.run(true,false, op);
    } catch ( InvocationTargetException e ) {
      // 同时操作锁提示 2017-11-28 zy
      if(e.getMessage().contains("Lock wait timeout exceeded")){
        MessageDialog.openInformation(shell, BaseMessages.getString( PKG, "ErrorDialog.ShowDetails.Tip" ),
                BaseMessages.getString( PKG, "ErrorDialog.ShowDetails.text" ));
      } else {
