  ...
  public boolean tabClose( TabItem item ) throws KettleException {
    ...
    for ( TabMapEntry entry : collection ) {
      if ( item.equals( entry.getTabItem() ) ) {
        ...
        try {
          //当用户关闭TAB，正在运行的任务就自动停止了。单号：168262
          if(item.getControl()!=null&&item.getControl() instanceof JobGraph){
            JobGraph jobGraph = ((JobGraph) item.getControl());
            if(jobGraph.getJob()!= null && jobGraph.getJob().isActive()){
              int reply = jobGraph.showRunning();
              if(reply ==SWT.YES){
                close = close&&true;
                jobGraph.stopJob();
              }else {
          ...
          }else if(item.getControl()!=null&&item.getControl() instanceof TransGraph){
            TransGraph transGraph=((TransGraph) item.getControl());
            if(transGraph!=null&&transGraph.getTrans().isRunning()){
              int reply = transGraph.showRunning();
              if(reply ==SWT.YES){
                close = close&&true;
                transGraph.stop();
              }else {
