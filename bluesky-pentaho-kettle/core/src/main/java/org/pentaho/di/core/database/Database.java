...
public class Database implements VariableSpace, LoggingObjectInterface {
	 ...
	 * @deprecated Please specify the parent object so that we can see which object
	 *             is initiating a database connection
	 */
	@Deprecated
	...
	/**
	 * This implementation is NullPointerException subject, and may not follow
	 * fundamental equals contract.
	 * <p/>
	 ...
	 * <code>DatabaseMeta.TYPE_ACCESS_JNDI</code>, then the connection's datasource
	 * is looked up in JNDI</li>
	 * <li>If <code>databaseMeta.isUsingConnectionPool()</code>, then the
	 * connection's datasource is looked up in the pool</li>
	 * <li>otherwise, the connection is established via
	 * {@linkplain DriverManager}</li>
	 * </ol>
	 *
	 * @param partitionId
	 *            the partition ID in the cluster to connect to.
					...
					if (databaseMeta.getDatabaseInterface() instanceof MSSQLServerNativeDatabaseMeta) {
						...
						if (Const.isEmpty(instance)) {
							...
							connection = DriverManager.getConnection(
									url + ";user=" + username + ";password=" + password + ";instanceName=" + instance);
						}
					} else {
	...
	public void setAutoCommit(boolean useAutoCommit) throws KettleDatabaseException {
		try {
			...
			if (useAutoCommit) {
				...
				throw new KettleDatabaseException(
						BaseMessages.getString(PKG, "Database.Exception.UnableToDisableAutoCommit", toString()));
			}
	...
	public void commitLog(boolean force, LogTableCoreInterface logTable) throws KettleDatabaseException {
		try {
			...
			DatabaseLogExceptionFactory.getExceptionStrategy(logTable).registerException(log, e, PKG,
					"Database.Error.UnableToCommitToLogTable", logTable.getActualTableName());
		}
	}

	/**
	 * this is a copy of {@link #commit(boolean)} - but delegates exception handling
	 * to caller. Can be possibly be removed in future.
	 *
	 * @param force
	 * @throws KettleDatabaseException
	 * @throws SQLException
	...
	public String setHiveValues(RowMetaInterface rowMeta, Object[] data, String sql, String seg) throws KettleException {
		StringBuilder sb = new StringBuilder(sql).append(seg).append("(");
		boolean first = true;
		for (int i = 0; i < rowMeta.size(); i++) {
			ValueMetaInterface v = rowMeta.getValueMeta(i);
			Object object = data[i];
			...
			try {
				String value = v.getHivePreparedStatementValue(databaseMeta, object);
				sb.append(value);
			} catch (KettleException e) {
		...
		try {
			if (pstmt_seq == null) {
				pstmt_seq = connection
						.prepareStatement(databaseMeta.getSeqNextvalSQL(databaseMeta.stripCR(schemaSequence)));
			}
			ResultSet rs = null;
			try {
	...
	public String getHiveInsertStatement(String tableName, RowMetaInterface fields) {
	...
	/**
	 * Insert a row into the database using a prepared statement that has all values
	 * set.
	 *
	 * @param ps
	 ...
	 * @return true if the rows are safe: if batch of rows was sent to the database
	 *         OR if a commit was done.
	 * @throws KettleDatabaseException
	 */
	 ...
	 *            The number of rows on the batch queue
	 * @throws KettleDatabaseException
	 * @deprecated use emptyAndCommit() instead (pass in the number of rows left in
	 *             the batch)
	 */
	@Deprecated
	...
	private boolean canWeSetFetchSize(Statement statement) throws SQLException {
		return databaseMeta.isFetchSizeSupported()
				&& (statement.getMaxRows() > 0 || databaseMeta.getDatabaseInterface() instanceof PostgreSQLDatabaseMeta
	...
	public boolean checkTableExists(String tablename) throws KettleDatabaseException {
		try {
			 ...
			 * getDatabaseMetaData().getTables(null, null, "%" , new String[] { "TABLE",
			 * "VIEW", "SYNONYM" } ); boolean found = false; if (alltables!=null) { while
			 * (alltables.next() && !found) { String schemaName =
			 * alltables.getString("TABLE_SCHEM"); String name =
			 * alltables.getString("TABLE_NAME"); if ( tablename.equalsIgnoreCase(name) || (
			 * schemaName!=null && tablename.equalsIgnoreCase(
			 ...
			 * "Unable to read table-names from the database meta-data."); } } else { throw
	...
	public boolean checkColumnExists(String columnname, String tablename) throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException("Unable to check if column [" + columnname + "] exists in table ["
					+ tablename + "] on connection [" + databaseMeta.getName() + "]", e);
		}
	}
	...
	public RowMetaInterface getQueryFields(String sql, boolean param, RowMetaInterface inform, Object[] data)
		...
		//
		//胡将， 去除缓存查找 -- 严重影响性能 。 2017年11月24日15:55:20
	/*	if (dbcache != null) {
		...
		try {
			...
			if (rs.next()) {
				for (int i = 0; i < nrcols; i++) {
					...
					//hive 的二进制格式不支持
					if("Binary".equalsIgnoreCase(val.getTypeDesc())){
						continue;
					}
	...
	/**
	 * Prepare a delete statement by giving it the tablename, fields and conditions
	 * to work with.
	 *
	 * @param table
	...
	public Object[] getLookup(PreparedStatement ps) throws KettleDatabaseException {
		// we assume this is external PreparedStatement and we may need to re-create
		// rowMeta
	...
	public String getAlterTableStatement(String tableName, RowMetaInterface fields, String tk, boolean use_autoinc,
		...
		if ("POSTGRESQL".equals(databaseMeta.getPluginId())) {
			for (int i = 0; i < tabFields.size(); i++) {
				tabFields.getValueMeta(i).setName('\"' + tabFields.getValueMeta(i).getName() + '\"');
			}
		}
		...
		try {
			...
			if (update) {
				...
				insertRow(environmentSubstitute(logTable.getActualSchemaName()),
						environmentSubstitute(logTable.getActualTableName()), logRecord.getRowMeta(),
						logRecord.getData());

			}
		} catch (Exception e) {
			DatabaseLogExceptionFactory.getExceptionStrategy(logTable).registerException(log, e, PKG,
					"Database.Error.WriteLogTable", environmentSubstitute(logTable.getActualTableName()));
		}
	}
	...
	public void cleanupLogRecords(LogTableCoreInterface logTable) throws KettleDatabaseException {
		...
		if (schemaTable.isEmpty()) {
			...
			DatabaseLogExceptionFactory.getExceptionStrategy(logTable).registerException(log, PKG,
					"DatabaseMeta.Error.LogTableNameNotFound");
		}
	...
	public Object[] getLastLogDate(String logtable, String name, boolean job, LogStatus status)
		...
		sql += " AND    " + jobtrans + " = ?";
		sql += " ORDER BY " + databaseMeta.quoteField("LOGDATE") + " DESC, " + databaseMeta.quoteField("ENDDATE")
				+ " DESC";
	...
	public Savepoint setSavepoint() throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(BaseMessages.getString(PKG, "Database.Exception.UnableToSetSavepoint"),
					e);
		}
	}
	...
	public Savepoint setSavepoint(String savePointName) throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(
					BaseMessages.getString(PKG, "Database.Exception.UnableToSetSavepointName", savePointName), e);
		}
	}
	...
	public String[] getPrimaryKeyColumnNames(String tablename) throws KettleDatabaseException {
		...
		try {
			...
			try {
				...
				throw new KettleDatabaseException(
						"Error closing connection while searching primary keys in table [" + tablename + "]", e);
			}
		}
	...
	/**
	 * Execute an SQL statement inside a file on the database connection (has to be
	 * open)
	 *
	 * @param sql
	...
	// 执行预编译语句
	public void execPreStatments(String sql) {
