...
public class RowMeta implements RowMetaInterface,Serializable {
   ...
   * @param fieldnrs
   * @param fieldValueMap
   * @param fieldVerifyMap
   * @return true相等 false不相等
   * @throws KettleValueException
   */
  @Override
  public boolean compare( Object[] rowData1, Object[] rowData2, int[] fieldnrs,
                      Map<String, String> fieldValueMap,
                      Map<String, Boolean> fieldVerifyMap) throws KettleValueException {
    lock.readLock().lock();
    try {
      for ( int fieldnr : fieldnrs ) {
        ValueMetaInterface valueMeta = getValueMeta( fieldnr );

        Object source = rowData1[fieldnr];
        int cmp = valueMeta.compare(source, rowData2[ fieldnr ] );
        if(valueMeta.isNull(source)){
          fieldValueMap.put(valueMeta.getName(), "");
        }else {
            fieldValueMap.put(valueMeta.getName(), valueMeta.getString(source).trim());
        }

        fieldVerifyMap.put(valueMeta.getName(), cmp == 0);
      }

        return fieldVerifyMap.values().stream().allMatch(a -> a);
    } finally {
      lock.readLock().unlock();
    }
  }
