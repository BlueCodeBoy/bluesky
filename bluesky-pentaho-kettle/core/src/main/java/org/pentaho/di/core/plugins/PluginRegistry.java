...
public class PluginRegistry {
  ...
  //TODO
  //region 修改类加载器，解决WebappClassLoader加载问题
  ...
  private URLClassLoader createClassLoader( PluginInterface plugin ) throws MalformedURLException,
          UnsupportedEncodingException {
    List<String> jarfiles = plugin.getLibraries();
    URL[] urls = new URL[jarfiles.size()];
    for (int i = 0; i < jarfiles.size(); i++) {
    ...
    ClassLoader classLoader = getClass().getClassLoader();
    System.out.println("createClassLoader:"+classLoader.getClass().getName());
    ...
    if ( plugin instanceof ClassLoadingPluginInterface ) {
      ...
      try {
        ...
        if ( plugin.isNativePlugin() ) {
          ...
          // Load the class.
          cl = (Class<? extends T>) ucl.getParent().loadClass( className );
        }
  ...
  public ClassLoader getClassLoader( PluginInterface plugin ) throws KettlePluginException {
    ...
    if("Mongo".contains(plugin.getName())){
