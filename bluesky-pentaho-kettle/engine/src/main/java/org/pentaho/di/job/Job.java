 ...
 * <p>
 * The definition of a PDI job is represented by a JobMeta object. It is
 * typically loaded from a .kjb file, a PDI repository, or it is generated
 * dynamically. The declared parameters of the job definition are then queried
 * using listParameters() and assigned values using calls to
 * setParameterValue(..).
 *
 * @author Matt Casters
 * @since 07-apr-2003
...
public class Job extends Thread implements VariableSpace, NamedParams, HasLogChannelInterface, LoggingObjectInterface,
	...
	public void run() {
		...
		try {
			...
			// Create a new variable name space as we want jobs to have their own set of
			// variables.
	...
	private void emergencyWriteJobTracker(Result res) {
		JobEntryResult jerFinalResult = new JobEntryResult(res, this.getLogChannelId(),
	...
	private Result execute() throws KettleException {
		try {
			...
			// Start the tracking...
			JobEntryResult jerStart = new JobEntryResult(null, null,
					BaseMessages.getString(PKG, "Job.Comment.JobStarted"),
			...
			if (startpoint.isStart()) {
				...
				jerEnd = new JobEntryResult(res, jes.getLogChannelId(),
	...
	private Result execute(final int nr, Result prev_result, final JobEntryCopy jobEntryCopy, JobEntryCopy previous,
		...
		// region License验证
		/*
		 * if(!LincenseManager.Instance().verify()) {
		 * log.logError("Job.execute - - - - license验证失败!"); res = new Result( nr );
		...
		if (!extension.executeEntry) {
			...
			JobEntryResult jerBefore = new JobEntryResult(null, null,
					BaseMessages.getString(PKG, "Job.Comment.JobStarted"), reason, jobEntryCopy.getName(),
			...
			JobEntryResult jerAfter = new JobEntryResult(newResult, cloneJei.getLogChannel().getLogChannelId(),
					BaseMessages.getString(PKG, "Job.Comment.JobFinished"), null, jobEntryCopy.getName(),
					jobEntryCopy.getNr(), environmentSubstitute(jobEntryCopy.getEntry().getFilename()));
		...
		// next 2 lists is being modified concurrently so must be synchronized for this
		// case.
		...
		for (int i = 0; i < nrNext && !isStopped(); i++) {
			...
			if (hi.isUnconditional() || (jobEntryCopy.evaluates() && (!(hi.getEvaluation() ^ newResult.getResult())))) {
				...
				if (jobEntryCopy.isLaunchingInParallel()) {
						...
						public void run() {
							try {
								...
								threadExceptions.add(new KettleException(
										BaseMessages.getString(PKG, "Job.Log.UnexpectedError", nextEntry.toString()),
										e));
					...
					if (log.isBasic()) {
						log.logBasic(
								BaseMessages.getString(PKG, "Job.Log.LaunchedJobEntryInParallel", nextEntry.getName()));
					}
				} else {
					try {
					...
					if (log.isBasic()) {
						log.logBasic(BaseMessages.getString(PKG, "Job.Log.FinishedJobEntry", nextEntry.getName(),
								res.getResult() + ""));
					}
				}
			}
	 ...
	 *
	 * @return nr of error that have occurred during execution. During execution of
	 *         a job the number can change.
	 */
	...
	public boolean beginProcessing() throws KettleException {
		...
		if (jobLogTable.isDefined()) {
			...
			try {
				...
				if (jobMeta.getJobLogTable().isBatchIdUsed()) {
					// id_batch = logcon.getNextBatchId( ldb, schemaName, tableName,
					// jobLogTable.getKeyField().getFieldName() );
					id_batch = System.nanoTime();
					...
					TimerTask timerTask = new TimerTask() {
						public void run() {
							try {
								...
								log.logError(
										BaseMessages.getString(PKG, "Job.Exception.UnableToPerformIntervalLogging"), e);
								// Also stop the show...
								//
				...
				addJobListener(new JobAdapter() {
					public void jobFinished(Job job) throws KettleException {
						try {
							...
							log.logError(BaseMessages.getString(PKG, "Job.Exception.UnableToWriteToLoggingTable",
									jobLogTable.toString()), e);
		...
		// If we need to write out the job entry logging information, do so at the end
		// of the job:
		//
			...
			addJobListener(new JobAdapter() {
				public void jobFinished(Job job) throws KettleException {
					try {
						...
						throw new KettleException(
								BaseMessages.getString(PKG, "Job.Exception.UnableToPerformJobEntryLoggingAtJobEnd"), e);
					}
				}
			});
		}
	...
	private boolean endProcessing() throws KettleJobException {
		...
		try {
			...
			if (jobLogTable.isDefined()) {
				...
				// hujiang 在存入之后删除 日志缓存 2017年11月28日19:59:51
				if (this.getParentJob() == null) {
	...
	/**
	 * Writes information to Job Log table. Cleans old records, in case job is
	 * finished.
	 *
	 */
	...
	protected void writeLogChannelInformation() throws KettleException {
		...
		// PDI-7070: If parent job has the same channel logging info, don't duplicate
		// log entries
		Job j = getParentJob();
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.variables.VariableSpace#copyVariablesFrom(org.pentaho.di.
	 * core.variables.VariableSpace)
	 */
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.variables.VariableSpace#environmentSubstitute(java.lang.
	 * String)
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.pentaho.di.core.variables.VariableSpace#setParentVariableSpace(org.
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.variables.VariableSpace#getVariable(java.lang.String,
	 * java.lang.String)
	 */
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.variables.VariableSpace#getBooleanValueOfVariable(java.
	 * lang.String, boolean)
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.pentaho.di.core.variables.VariableSpace#initializeVariablesFrom(org.
	 * pentaho.di.core.variables.VariableSpace)
	 */
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.variables.VariableSpace#setVariable(java.lang.String,
	 * java.lang.String)
	 */
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.variables.VariableSpace#shareVariablesWith(org.pentaho.di
	 * .core.variables.VariableSpace)
	 */
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.variables.VariableSpace#injectVariables(java.util.Map)
	 */
	...
	public static String sendToSlaveServer(JobMeta jobMeta, JobExecutionConfiguration executionConfiguration,
		...
		try {
			...
			if (executionConfiguration.isPassingExport()) {
				...
				//
				FileObject tempFile = KettleVFS.createTempFile("jobExport", ".zip",
						System.getProperty("java.io.tmpdir"), jobMeta);
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.parameters.NamedParams#addParameterDefinition(java.lang.
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.parameters.NamedParams#getParameterDescription(java.lang.
	 * String)
	 */
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.parameters.NamedParams#getParameterDefault(java.lang.
	 * String)
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.pentaho.di.core.parameters.NamedParams#getParameterValue(java.lang.
	 * String)
	 */
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.pentaho.di.core.parameters.NamedParams#setParameterValue(java.lang.
	 ...
	 *
	 * @see
	 * org.pentaho.di.core.parameters.NamedParams#copyParametersFrom(org.pentaho.di.
	 * core.parameters.NamedParams)
	 */
	...
	/**
	 * Gets a flat list of results in THIS job, in the order of execution of job
	 * entries.
	 *
	 * @return A flat list of results in THIS job, in the order of execution of job
	 *         entries
	 */
		...
		heartbeat.scheduleAtFixedRate(new Runnable() {
			public void run() {
				...
				try {
					...
					log.logDebug("Triggering heartbeat signal for " + jobMeta.getName() + " at every "
	...
	private int getHeartbeatIntervalInSeconds() {
		...
		// 1 - check if there's a user defined value ( job-specific ) heartbeat periodic
		// interval;
		// 2 - check if there's a default defined value ( job-specific ) heartbeat
		// periodic interval;
