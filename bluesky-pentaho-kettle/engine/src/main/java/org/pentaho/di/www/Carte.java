...
public class Carte {
  ...
  private static void parseAndRunCommand( String[] args ) throws Exception {
    ...
    if ( arguments.length == 1 && !Const.isEmpty( arguments[0] ) ) {
      if ( cmd.hasOption( 's' ) ) {
        throw new CarteCommandException( BaseMessages.getString( PKG, "Carte.Error.illegalStop" ) );
      }
  ...
  private static void callStopCarteRestService( String hostname, String port, String username, String password )
    ...
    try {
      ...
      if ( response == null || !response.contains( "<serverstatus>" ) ) {
        throw new CarteCommandException( BaseMessages.getString( PKG, "Carte.Error.NoServerFound", hostname, ""
            + port ) );
      }
      ...
      if ( response == null || !response.contains( "Shutting Down" ) ) {
        throw new CarteCommandException( BaseMessages.getString( PKG, "Carte.Error.NoShutdown", hostname, ""
            + port ) );
      }
    } catch ( Exception e ) {
      throw new CarteCommandException( BaseMessages.getString( PKG, "Carte.Error.NoServerFound", hostname, ""
