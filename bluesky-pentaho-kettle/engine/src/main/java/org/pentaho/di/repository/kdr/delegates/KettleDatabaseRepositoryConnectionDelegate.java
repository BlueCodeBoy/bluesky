...
public class KettleDatabaseRepositoryConnectionDelegate extends KettleDatabaseRepositoryBaseDelegate {
	private static final Object generateIdObject = new Object();
	...
	private void setPrepareStatementNull(){
	  psStepAttributesLookup = null;
	  psStepAttributesInsert = null;
	  psTransAttributesLookup = null;
	...
	public synchronized void connect(boolean no_lookup, boolean ignoreVersion) throws KettleException {
		...
		try {
			 ...
			 * repository.connectionDelegate.setLookupJobAttribute(); } catch
			 * (KettleException dbe) { throw new
	...
	protected void verifyVersion() throws KettleException {
		...
		String versionTable = databaseMeta.getQuotedSchemaTableCombination(null,
				KettleDatabaseRepository.TABLE_R_VERSION);
		try {
			lastUpgrade = database.getOneRow("SELECT " + quote(KettleDatabaseRepository.FIELD_VERSION_MAJOR_VERSION)
			...
			try {
				...
				//
				String userTable = databaseMeta.getQuotedSchemaTableCombination(null,
		...
		if (majorVersion == 3 && minorVersion == 0) {
			...
			//
			String tableName = databaseMeta.getQuotedSchemaTableCombination(null,
	...
	public synchronized void closeLookupPreparedStatement() {
		try {
	...
	public synchronized void fillStepAttributesBuffer(ObjectId id_transformation) throws KettleException {
		...
		try {
			...
			/*
			 * if ( ps == null ) { ps = database.prepareSQL( sql ); sqlMap.put( sql, ps ); }
			 */

			RowMetaAndData parameter = getParameterMetaData(id_transformation);
			resultSet = database.openQuery(ps, parameter.getRowMeta(), parameter.getData());
	...
	public  void  checkPsClosed(){
		try {
			if(psJobAttributesLookup!=null&&psJobAttributesLookup.isClosed()){
				psJobAttributesLookup = null;
			}
		}catch (SQLException e){
			psJobAttributesLookup = null;
		...
		try {
			if(psTransAttributesLookup!=null&&psTransAttributesLookup.isClosed()){
				psTransAttributesLookup = null;
			}
		}catch (SQLException e){
			psTransAttributesLookup = null;
	...
	public synchronized int countNrStepAttributes(ObjectId id_step, String code) throws KettleException {
	    ...
	    if (stepAttributesBuffer != null) {
			...
			table.addValue(
					new ValueMeta(KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_CODE, ValueMetaInterface.TYPE_STRING),
					code);
	...
	public synchronized int countNrTransAttributes(ObjectId id_transformation, String code) throws KettleException {
		...
		table.addValue(new ValueMeta(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_ID_TRANSFORMATION,
				ValueMetaInterface.TYPE_INTEGER), id_transformation);
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_CODE, ValueMetaInterface.TYPE_STRING),
				code);
		RowMetaAndData r = database.getOneRow(sql, table.getRowMeta(), table.getData());
		if (r == null || r.getData() == null) {
			return 0;
	...
	public synchronized List<Object[]> getTransAttributes(ObjectId id_transformation, String code, long nr)
		...
		table.addValue(
				...
				code);
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_NR, ValueMetaInterface.TYPE_INTEGER),
				new Long(nr));
	...
	public synchronized ObjectId insertJobEntryAttribute(ObjectId id_job, ObjectId id_jobentry, long nr, String code,
		...
		table.addValue(
				...
				new Long(nr));
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_JOBENTRY_ATTRIBUTE_CODE, ValueMetaInterface.TYPE_STRING),
				code);
	...
	public synchronized LongObjectId getNextID(String tableName, String fieldName) throws KettleException {
		synchronized (generateIdObject) {
			...
			// return new LongObjectId( counter.next() );
			// }
			// return new LongObjectId(System.currentTimeMillis());
			return getNextSequence(tableName, fieldName);
		}
	}

	private synchronized LongObjectId getNextSequence(String tablename, String idfield) throws KettleException {
        if (!isValid()) {
            reConnection();
        }
		LongObjectId retval = null;

		RowMetaAndData r = database.getOneRow("SELECT nextval('" + tablename + "." + idfield + "') ;");
		...
		log.logRowlevel("tablename:" + tablename + ",id:" + retval);
		return retval;
	}

	/*
	 * private synchronized LongObjectId getNextTableID( String tablename, String
	 * idfield ) throws KettleException { LongObjectId retval = null;
	 * 
	 * RowMetaAndData r = database.getOneRow( "SELECT MAX(" + idfield + ") FROM " +
	 * tablename ); if ( r != null ) { Long id = r.getInteger( 0 );
	 * 
	 * if ( id == null ) { if ( log.isDebug() ) { log.logDebug( "no max(" + idfield
	 * + ") found in table " + tablename ); } retval = new LongObjectId( 1 ); } else
	 * { if ( log.isDebug() ) { log.logDebug( "max(" + idfield + ") found in table "
	 * + tablename + " --> " + idfield + " number: " + id ); } retval = new
	 * LongObjectId( id.longValue() + 1L ); } } return retval; }
	 */
	...
	public synchronized void setLookupJobEntryAttribute() throws KettleException {
	    ...
	    String sql = "SELECT " + quote(KettleDatabaseRepository.FIELD_JOBENTRY_ATTRIBUTE_VALUE_STR) + ", "
	...
	public synchronized ObjectId insertStepAttribute(ObjectId id_transformation, ObjectId id_step, long nr, String code,
		...
		table.addValue(
				...
				code);
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_VALUE_NUM, ValueMetaInterface.TYPE_NUMBER),
				new Double(value_num));
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_VALUE_STR, ValueMetaInterface.TYPE_STRING),
				value_str);

		/*
		 * If we have prepared the insert, we don't do it again. We assume that all the
		 * step insert statements come one after the other.
		 */

		if (psStepAttributesInsert == null) {
			String sql = database.getInsertStatement(KettleDatabaseRepository.TABLE_R_STEP_ATTRIBUTE,
					table.getRowMeta());
	...
	public synchronized ObjectId insertTransAttribute(ObjectId id_transformation, long nr, String code, long value_num,
		...
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_VALUE_STR, ValueMetaInterface.TYPE_STRING),
				value_str);

		/*
		 * If we have prepared the insert, we don't do it again. We asume that all the
		 * step insert statements come one after the other.
		 */

		if (psTransAttributesInsert == null) {
			String sql = database.getInsertStatement(KettleDatabaseRepository.TABLE_R_TRANS_ATTRIBUTE,
					table.getRowMeta());
	...
	public synchronized ObjectId insertJobAttribute(ObjectId id_job, long nr, String code, long value_num,
		...
		// System.out.println("Insert job attribute : id_job="+id_job+", code="+code+",
		// value_str="+value_str);
		...
		table.addValue(new ValueMeta(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_CODE, ValueMetaInterface.TYPE_STRING),
				code);
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_NUM, ValueMetaInterface.TYPE_INTEGER),
				new Long(value_num));
		table.addValue(
				new ValueMeta(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_STR, ValueMetaInterface.TYPE_STRING),
				value_str);
		...
		if (psJobAttributesInsert == null) {
			String sql = database.getInsertStatement(KettleDatabaseRepository.TABLE_R_JOB_ATTRIBUTE,
					table.getRowMeta());
		...
		try {
			...
			if (rs != null) {
				...
				for (Object[] r : rows) {
					...
					repositoryObjects.add(new RepositoryObject(id, rowMeta.getString(r, 0), repositoryDirectory,
							rowMeta.getString(r, 1), rowMeta.getDate(r, 2), objectType, rowMeta.getString(r, 3),
							false));
				}
			}
	...
	public synchronized void lockRepository() throws KettleException {
	    ...
	    if (database.getDatabaseMeta().needsToLockAllTables()) {
			...
			database.lockTables(
					getQuotedSchemaTablenames(new String[] { KettleDatabaseRepository.TABLE_R_REPOSITORY_LOG, }));
		}
	}
	...
	public synchronized LongObjectId getIDWithValue(String tablename, String idfield, String lookupfield, String value)
		...
		RowMetaAndData result = getOneRow("SELECT " + idfield + " FROM " + tablename + " WHERE " + lookupfield + " = ?",
	...
	static String createIdsWithValuesQuery(String tablename, String idfield, String lookupfield, int amount) {
		...
		sb.append("SELECT ").append(idfield).append(" FROM ").append(tablename).append(" WHERE ").append(lookupfield)
				.append(" IN (");
	...
	public Map<String, LongObjectId> getValueToIdMap(String tablename, String idfield, String lookupfield)
			throws KettleException {
		String sql = new StringBuilder("SELECT ").append(lookupfield).append(", ").append(idfield).append(" FROM ")
				.append(tablename).toString();

		Map<String, LongObjectId> result = new HashMap<String, LongObjectId>();
		for (Object[] row : database.getRows(sql, new RowMeta(), new Object[] {}, ResultSet.FETCH_FORWARD, false, -1,
				null)) {
	...
	public LongObjectId[] getIDsWithValues(String tablename, String idfield, String lookupfield, String[] values)
			throws KettleException {
	...
	public synchronized ObjectId getIDWithValue(String tablename, String idfield, String lookupfield, String value,
		...
		RowMetaAndData result = getOneRow(
				"SELECT " + idfield + " FROM " + tablename + " WHERE " + lookupfield + " = ? AND " + lookupkey + " = ?",
				par.getRowMeta(), par.getData());
	...
	public Collection<RowMetaAndData> getDatabaseAttributes(ObjectId id_database)
		...
		try {
			/*
			 * f (ps == null) { ps = database.prepareSQL(sql); sqlMap.put(sql, ps); }
			 */
			ps = database.prepareSQL(sql);
			// Assemble the parameter (if any)
			//
			...
			try {
				...
				List<Object[]> rows = database.getRows(resultSet, 0, null);
				for (Object[] row : rows) {
					RowMetaAndData rowWithMeta = new RowMetaAndData(repository.connectionDelegate.getReturnRowMeta(),
							row);
	...
	public void performDelete(String sql, ObjectId... ids) throws KettleException {
		...
		try {
			...
			// 胡将更改，2017年11月9日17:40:25。锁表的问题
			/*
			 * if (e.getMessage().contains("Deadlock found when trying to get lock")) { try
			 * { Thread.currentThread().sleep(1000);
			 * 
			 * try { ps = database.prepareSQL(sql); RowMetaAndData param =
			 * getParameterMetaData(ids); database.setValues(param, ps); ps.execute(); }
			 * catch (SQLException e1) { throw new KettleException(
			 * "Unable to perform delete with SQL: " + sql + ", ids=" + ids.toString(), e1);
			 * }
			 * 
			 * } catch (InterruptedException e1) { // e1.printStackTrace(); throw new
			 * KettleException("Wait dataBase unlock... ", e1); } } else { throw new
			 * KettleException("Unable to perform delete with SQL: " + sql + ", ids=" +
			 * ids.toString(), e); }
			 */
		} finally {
			if (null != ps) {
	...
	/**
	 * A MySQL InnoDB hack really... Doesn't like a lock in case there's been a read
	 * in another session. It considers it an open transaction.
	 *
	 * @throws KettleDatabaseException
	 */
    ...
    private void reConnection() throws KettleDatabaseException{
        int reTryTime = 3;
        try {
            String[] type = {"SYSTEM TABLE"};
            database.getConnection().getMetaData().getTables(null, null, null, type);
        } catch (Exception e) {
			/* if can not connect to database, then retry 3 times,
			   if all the result of retry are failed,
            ...
            for (int i = 1; i <= reTryTime; i++) {
                try {
                  ...
                  //  database.getConnection();
                    ...
                    if (i == reTryTime) {
                        throw new KettleDatabaseException("Can not connect to database");
                    }
                    try {
