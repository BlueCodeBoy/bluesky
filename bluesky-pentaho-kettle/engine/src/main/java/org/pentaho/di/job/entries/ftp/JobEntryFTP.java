...
public class JobEntryFTP extends JobEntryBase implements Cloneable, JobEntryInterface {
  ...
  public JobEntryFTP( String name ) {
    super( name, "" );
    jobName=name;
    nr_limit = "10";
    port = "21";
  ...
  public Result execute( Result previousResult, int nr ) {
    ...
    try {
      ...
      ftpclient.setRemoteAddr( getInetAddress( realServername ) );//获取远程服务器IP地址
...
//      }
  ...
  private void downloadFile( FTPClient ftpClient, String fileName, String realMoveToFolder, Job parentJob,
    ...
    /*int lastPos = 0;
    FtpBreakInfoList.FtpBreakInfo info = KettleClientEnvironment.getInstance().getFtpBreakInfoList().findFtpDownBreakInfoByKey(jobName);
    if (info != null) {
      if (info.getFileFullName().equals(filename) && info.getBreakPos() > 0) {
        lastPos = info.getBreakPos();
      }
    } else {
      info = KettleClientEnvironment.getInstance().getFtpBreakInfoList().new FtpBreakInfo("", filename, 1);
    }
      File f = new File("c:\\ftp\\destData\\" + filename);
      FileOutputStream out = new FileOutputStream(f, true);
      int c;
      ftpclient.restart(lastPos);
      byte[] bytes = new byte[Const.BREAK_BLOCK_SIZE+1];
      //FileInputStream in = new FileInputStream(new String(filename.getBytes("GBK"), "iso-8859-1"));
    bytes =ftpclient.get(new String(filename.getBytes("GBK"),"iso-8859-1"));
    try {
        while (bytes.length>0) {
          out.write(bytes, lastPos, bytes.length);
          lastPos += bytes.length;
          info.setBreakPos(lastPos);
          KettleClientEnvironment.getInstance().getFtpBreakInfoList().addFtpDownBreakInfo(jobName,info);
          ftpclient.restart(lastPos);
          bytes =ftpclient.get(new String(filename.getBytes("GBK"),"iso-8859-1"));
        }
        KettleClientEnvironment.getInstance().getFtpBreakInfoList().removeFtpDownBreakInfo(jobName);
    }
      catch ( Exception e ) {
        logError( BaseMessages.getString( PKG, "JobEntryFTP.ErrorQuitting", e.getMessage() ) );
      }
      ...
      long lRemoteSize = ftpClient.size(fileName);
      String realLocalDirectory = environmentSubstitute( targetDirectory );
      File f = new File( realLocalDirectory + Const.FILE_SEPARATOR +fileName);
      if (f.exists()) {
        OutputStream out = new FileOutputStream(f, true);
        if (f.length() >= lRemoteSize) {
          logDetailed(BaseMessages.getString(PKG, "JobEntryFTP.GotFile", fileName + "本地文件大小大于远程文件大小，下载中止"));
        }
        else {
          ftpClient.restart(f.length());
          ftpClient.setDataReceiveBufferSize(Const.BREAK_BLOCK_SIZE);
