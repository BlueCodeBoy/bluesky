...
public class KettleDatabaseRepositoryTransDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  public void saveTransformation( TransMeta transMeta, String versionComment, ProgressMonitorListener monitor,
    ...
    try {
    	  //update by hujiang，2018年04月28日14:06:01 判断当前的存储目录为"/"，提示不能保存，需去web端添加目录
/*    	  if(transMeta.getRepositoryDirectory().getName().equals("/")) {
    		  throw new KettleException(BaseMessages.getString(PKG,"KettleDatabaseRepositoryTransDelegate.Save.isRootDir"));
    	  }*/
      if ( monitor != null ) {
      ...
      if ( transMeta.getObjectId() == null ) {
        ...
        //屏蔽在总方法中进行提交 2017-11-17 by llj
        //repository.deleteTransformation( transMeta.getObjectId());
        repository.deleteTransformationWithTransaction( transMeta.getObjectId() ,false);
      ...
      for ( int i = 0; i < transMeta.nrDatabases(); i++ ) {
        ...
        if ( overwriteAssociated || databaseMeta.hasChanged() || databaseMeta.getObjectId() == null ) {
          //屏蔽调用公共方法 2017-11-17 by llj
          //repository.save( databaseMeta, versionComment, monitor, overwriteAssociated );
          repository.databaseDelegate.saveDatabaseMeta(databaseMeta);
          //2017-11-17 by llj end
        }
        if ( monitor != null ) {
          monitor.worked( 1 );
      ...
      for ( int i = 0; i < transMeta.getPartitionSchemas().size(); i++ ) {
        ...
        /*repository.save(
          ...
          overwriteAssociated );*/
        repository.partitionSchemaDelegate.savePartitionSchema(partitionSchema,
                transMeta.getObjectId(), isUsedByTransformation, overwriteAssociated );
        //2017-11-17 by llj end
      }
      ...
      for ( int i = 0; i < transMeta.getSlaveServers().size(); i++ ) {
        ...
        /*repository.save(
          ...
          overwriteAssociated );*/
        repository.slaveServerDelegate.saveSlaveServer(slaveServer,
                transMeta.getObjectId(), isUsedByTransformation, overwriteAssociated );
      ...
      for ( int i = 0; i < transMeta.getClusterSchemas().size(); i++ ) {
        ...
        /*repository.save(
          ...
          overwriteAssociated );*/
        repository.clusterSchemaDelegate.saveClusterSchema(clusterSchema,
                versionComment, transMeta.getObjectId(), isUsedByTransformation, overwriteAssociated );
        //2017-11-17 by llj end
      }
