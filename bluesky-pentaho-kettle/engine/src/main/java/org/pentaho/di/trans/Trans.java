...
/**
 * This class represents the information and operations associated with the
 * concept of a Transformation. It loads, instantiates, initializes, runs, and
 * monitors the execution of the transformation contained in the specified
 * TransInfo object.
 *
 * @author Matt
 * @since 07-04-2003
...
public class Trans implements VariableSpace, NamedParams, HasLogChannelInterface, LoggingObjectInterface,
	...
	/**
	 * The job that's launching this transformation. This gives us access to the
	...
	/**
	 * The parent logging object interface (this could be a transformation or a
	 * job).
	 */
	private LoggingObjectInterface parent;

	/**
	 * The name of the mapping step that executes this transformation in case this
	 * is a mapping.
	 */
	...
	/**
	 * The date objects for logging information about the transformation such as
	 ...
	 * nothing is passed, it's the transformation's batch id.
	 */
	...
	/**
	 * The replayDate indicates that this transformation is a replay transformation
	 * for a transformation executed on replayDate. If replayDate is null, the
	 * transformation is not a replay.
	 */
	...
	/**
	 * Constant specifying a filename containing XML to inject into a ZIP file
	 * created during resource export.
	 */
	...
	/**
	 * The number of errors that have occurred during execution of the
	 * transformation.
	 */
	...
	/**
	 * In case this transformation starts to delegate work to a local transformation
	 * or job
	 */
	...
	/**
	 * Initializes a transformation from transformation meta-data defined in memory.
	 * Also take into account the parent log channel interface (job or
	 * transformation) for logging lineage purposes.
	 *
	 * @param transMeta
	 *            the transformation meta-data to use.
	 * @param parent
	 ...
	 * @see HasLogChannelInterface#getLogChannel()
	 */
	...
	/**
	 * Instantiates a new transformation using any of the provided parameters
	 * including the variable bindings, a repository, a name, a repository directory
	 * name, and a filename. This is a multi-purpose method that supports loading a
	 * transformation from a file (if the filename is provided but not a repository
	 * object) or from a repository (if the repository object, repository directory
	 * name, and transformation name are specified).
	 *
	 * @param parent
	 ...
	 *             if any error occurs during loading, parsing, or creation of the
	 *             transformation
	 */
	public <Parent extends VariableSpace & NamedParams> Trans(Parent parent, Repository rep, String name,
		...
		try {
			if (rep != null) {
				...
				if (repdir != null) {
					...
					throw new KettleException(
							BaseMessages.getString(PKG, "Trans.Exception.UnableToLoadTransformation", name, dirname));
				}
			} else {
			...
			throw new KettleException(BaseMessages.getString(PKG, "Trans.Exception.UnableToOpenTransformation", name),
					e);
		}
	}

	/**
	 * Executes the transformation. This method will prepare the transformation for
	 * execution and then start all the threads associated with the transformation
	 * and its steps.
	 *
	 * @param arguments
	 *            the arguments
	...
	public void execute(String[] arguments) throws KettleException {
		/*
		 * if(!LincenseManager.Instance().verify()){ log.logError(
		 * "Trans.execute - - - - license验证失败!"); return; }
		 */
	 ...
	 * arguments and parameters as well as preparing and tracking the steps and hops
	...
	public void prepareExecution(String[] arguments) throws KettleException {
		...
		if (transMeta.getName() == null) {
			...
			log.logBasic(
					BaseMessages.getString(PKG, "Trans.Log.DispacthingStartedForTransformation", transMeta.getName()));
		}
		...
		if (getReplayDate() != null) {
			...
			log.logBasic(
					BaseMessages.getString(PKG, "Trans.Log.ThisIsAReplayTransformation") + df.format(getReplayDate()));
		} else {
			if (log.isDetailed()) {
		...
		for (int i = 0; i < hopsteps.size(); i++) {
			...
			for (int n = 0; n < nrTargets; n++) {
				...
				if (thisCopies < 0) {
					// This can only happen if a variable is used that didn't resolve to a positive
					// integer value
					//
					throw new KettleException(
							BaseMessages.getString(PKG, "Trans.Log.StepCopiesNotCorrectlyDefined", thisStep.getName()));
				}
		...
		// Do a topology sort... Over 150 step (copies) things might be slowing down too
		// much.
		//
		...
		for (int i = 0; i < initThreads.length; i++) {
			...
			if (!initThreads[i].isOk()) {
				log.logError(
						BaseMessages.getString(PKG, "Trans.Log.StepFailedToInit", combi.stepname + "." + combi.copy));
	...
	private void checkCompatibility() {
		// If we don't have a previous result and transMeta does have one, someone has
		// been using a deprecated method.
		//
		if (transMeta.getPreviousResult() != null && getPreviousResult() == null) {
			setPreviousResult(transMeta.getPreviousResult());
		}

		// If we don't have arguments set and TransMeta has, someone has been using a
		// deprecated method.
		//
	...
	/**
	 * Starts the threads prepared by prepareThreads(). Before you start the
	...
	public void startThreads() throws KettleException {
						...
						if (nrOfFinishedSteps >= steps.size()) {
							...
							try {
								...
								log.logError(getName() + " : "
										+ BaseMessages.getString(PKG, "Trans.Log.UnexpectedErrorAtTransformationEnd"),
										e);
							}
						}
		...
		TransListener transListener = new TransAdapter() {
			public void transFinished(Trans trans) {
				...
				// If the user ran with metrics gathering enabled and a metrics logging table is
				// configured, add another
				// listener...
				//
				...
				// This will commit or roll back the transaction based on the result of this
				// transformation.
				//
		...
		// This should always be done first so that the other listeners achieve a clean
		// state to start from (setFinished and
		// so on)
		//
				...
				combi.step.addStepListener(new StepAdapter() {
					...
					public void stepFinished(Trans trans, StepMeta stepMeta, StepInterface step) {
						try {
							...
							throw new RuntimeException("Unexpected error in calling extension point upon step finish",
									e);
						}
					}
		...
		if (transMeta.isCapturingStepPerformanceSnapShots() && !pausedAndNotEmpty && !stoppedAndNotEmpty) {
			...
			for (int i = 0; i < steps.size(); i++) {
				...
				StepPerformanceSnapShot snapShot = new StepPerformanceSnapShot(seqNr, getBatchId(), new Date(),
						...
						step.getLinesInput(), step.getLinesOutput(), step.getLinesUpdated(), step.getLinesRejected(),
						step.getErrors());
	...
	/**
	 * This method performs any cleanup operations, typically called after the
	 * transformation has finished. Specifically, after ALL the slave
	 * transformations in a clustered run have finished.
	 */
	public void cleanup() {
	...
	/**
	 * Gets the number of errors that have occurred during execution of the
	 * transformation.
	 *
	...
	/**
	 * Gets the number of steps in the transformation that are in an end state, such
	 * as Finished, Halted, or Stopped.
	 *
	...
	/**
	 * Attempts to stops all running steps and subtransformations. If all steps have
	 * finished, the transformation is marked as Finished.
	 */
	public void killAll() {
		if (steps == null) {
			return;
	...
	/**
	 * Asks all steps to stop but doesn't wait around for it to happen. This is a
	...
	/**
	 * Logs the execution statistics for the transformation for the specified time
	 * interval. If the total length of execution is supplied as the interval, then
	 * the statistics represent the average throughput (lines
	...
	public void printStats(int seconds) {
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			if (seconds != 0) {
				if (step.getErrors() == 0) {
					log.logBasic(BaseMessages.getString(PKG, "Trans.Log.ProcessSuccessfullyInfo", step.getStepname(),
				...
				if (step.getErrors() == 0) {
					log.logBasic(BaseMessages.getString(PKG, "Trans.Log.ProcessSuccessfullyInfo", step.getStepname(),
							"." + step.getCopy(), String.valueOf(proc),
	...
	public RowSet findRowSet(String rowsetname) {
		...
		for (int i = 0; i < rowsets.size(); i++) {
			// log.logDetailed("DIS: looking for RowSet ["+rowsetname+"] in nr "+i+" of
	 ...
	 *
	 * @return a boolean array associated with the step list, indicating whether
	 *         that step is running a lookup.
	 */
	 ...
	 *
	 * @return an array associated with the step list, indicating the status of that
	 *         step.
	 */
	 ...
	 * @return a StepInterface object corresponding to the run thread for the
	 *         specified step
	 */
	...
	public void calculateBatchIdAndDateRange() throws KettleTransException {
		...
		try {
			if (logConnection != null) {
				...
				if (transLogTable.isBatchIdUsed()) {
					// Long id_batch = logConnection.getNextBatchId(transLogTableDatabaseConnection,
					// logSchema, logTable,transLogTable.getKeyField().getFieldName());
					// setBatchId( id_batch.longValue() );
					setBatchId(System.nanoTime());
				}

				//
				// Get the date range from the logging table: from the last end_date to now.
				// (currentDate)
				//

				Object[] lastr = transLogTableDatabaseConnection.getLastLogDate(logSchemaAndTable, transMeta.getName(),
						false, LogStatus.END);

				// region ketlle未找到数据时的处理机制 llj by 2017-11-2
				...
				// endregion 屏蔽kettle日志开始时间的处理机制结束
					...
					if (maxcon != null) {
						...
						try {
							...
							throw new KettleTransException(BaseMessages.getString(PKG,
									"Trans.Exception.ErrorConnectingToDatabase", "" + transMeta.getMaxDateConnection()),
									e);
						} finally {
							maxdb.disconnect();
						}
				...
				if (transMeta.nrDependencies() > 0) {
					...
					//
					// Maybe one of the tables where this transformation is dependent on has
					// changed?
						...
						if (depcon != null) {
							...
							try {
								...
								if (r1 != null) {
									...
									if (maxvalue != null) {
										if (log.isDetailed()) {
											log.logDetailed(BaseMessages.getString(PKG, "Trans.Log.FoundDateFromTable",
													td.getTablename(), "." + td.getFieldname(),
													" = " + maxvalue.toString()));
										}
			...
			if (transMeta.getMaxDateDifference() > 0.0 && // Do we have a difference specified?
				...
				// If this is the case: lower the end-date. Pick up the next 'region' next time
				// around.
				// We do this to limit the workload in a single update session (e.g. for large
				// fact tables)
				//
		...
		// It's closed later in beginProcessing() to prevent excessive
		// connect/disconnect repetitions.

	}

	/**
	 * Begin processing. Also handle logging operations related to the start of the
	 * transformation
	 *
	 * @throws KettleTransException
		...
		try {
			...
			try {
				if (transLogTableDatabaseConnection != null && !Const.isEmpty(logTable)
					...
					// transLogTableDatabaseConnection.writeLogRecord(transLogTable,
					// LogStatus.START, this, null);
					saveRecord(transLogTable, LogStatus.START, this, null);
					// Pass in a commit to release transaction locks and to allow a user to actually
					// see the log record.
					//
					...
					// Add a listener to make sure that the last record is also written when
					...
					addTransListener(new TransAdapter() {
						public void transFinished(Trans trans) throws KettleException {
							try {
								...
								throw new KettleException(
										BaseMessages.getString(PKG, "Trans.Exception.UnableToPerformLoggingAtTransEnd"),
										e);
							}
						}
					});
				...
				// If we need to write out the step logging information, do so at the end of the
				// transformation too...
				//
				...
				// If we need to write the log channel hierarchy and lineage information, add a
				// listener for that too...
				//
				...
				// If we use interval logging, we keep the connection open for performance
				// reasons...
				//
	...
	/**
	 * Writes log channel information to a channel logging table (if one has been
	 * configured).
	 *
	 * @throws KettleException
	...
	protected void writeLogChannelInformation() throws KettleException {
		...
		// PDI-7070: If parent trans or job has the same channel logging info, don't
	...
	/**
	 * Gets the result of the transformation. The Result object contains such
	 * measures as the number of errors, number of lines
	 * read/written/input/output/updated/rejected, etc.
	 *
	 * @return the Result object containing resulting measures from execution of the
	 *         transformation
	 */
	...
	/**
	 * End processing. Also handle any logging operations associated with the end of
	 * a transformation
	 *
		...
		if (logcon != null) {
			...
			try {
				...
				if (!Const.isEmpty(logTable)) {
					...
					//hujiang --2017年11月28日20:59:17
					if (this.getParentTrans() == null &&  this.getParentJob() == null) {
						String logChannelId = this.getLogChannelId();
						if(null != logChannelId && !"".equals(logChannelId))
	...
	/**
	 * Close unique database connections. If there are errors in the Result, perform
	 * a rollback
	 *
	 * @param result
	...
	private void closeUniqueDatabaseConnections(Result result) {
		...
		// Don't close any connections if the parent transformation is using the same
		// transaction
		//
			...
			if (result.getNrErrors() > 0) {
				for (DatabaseTransactionListener listener : transactionListeners) {
					try {
						...
						log.logError(
								BaseMessages.getString(PKG, "Trans.Exception.ErrorHandlingTransactionListenerRollback"),
								e);
						result.setNrErrors(result.getNrErrors() + 1);
					}
				}
				...
				for (DatabaseTransactionListener listener : transactionListeners) {
					try {
						...
						log.logError(
								BaseMessages.getString(PKG, "Trans.Exception.ErrorHandlingTransactionListenerCommit"),
								e);
						result.setNrErrors(result.getNrErrors() + 1);
					}
				}
	...
	/**
	 * Find the executing step copy for the step with the specified name and copy
	 * number
	 *
	 * @param stepname
	 *            the step name
	 ...
	 * @param stepname
	 *            the step name
	 * @return the list of executing step copies found or null if no steps are
	 *         available yet (incorrect usage)
	 */
	 ...
	 *
	 * @param monitored
	 *            true if the running transformation should be monitored, false
	 *            otherwise
	 */
	...
	/**
	 * Gets the dependency date for the transformation. A transformation can have a
	 * list of dependency fields. If any of these fields have a maximum date higher
	 * than the dependency date of the last run, the date range is set to to (-oo,
	 * now). The use-case is the incremental population of Slowly Changing
	 * Dimensions (SCD).
	 *
	...
	public String toString() {
		...
		// See if there is a parent transformation. If so, print the name of the parent
		// here as well...
		//
		StringBuffer string = new StringBuffer();

		// If we're running as a mapping, we get a reference to the calling (parent)
	...
	/**
	 * Gets the replay date. The replay date is used to indicate that the
	 * transformation was replayed (re-tried, run again) with that particular replay
	 * date. You can use this in Text File/Excel Input to allow you to save error
	 * line numbers into a file (SOURCE_FILE.line for example) During replay, only
	 * the lines that have errors in them are passed to the next steps, the other
	 * lines are ignored. This is for the use case: if the document contained errors
	 * (bad dates, chars in numbers, etc), you simply send the document back to the
	 * source (the user/departement that created it probably) and when you get it
	 * back, re-run the last transformation.
	 *
	...
	/**
	 * Sets the replay date. The replay date is used to indicate that the
	 * transformation was replayed (re-tried, run again) with that particular replay
	 * date. You can use this in Text File/Excel Input to allow you to save error
	 * line numbers into a file (SOURCE_FILE.line for example) During replay, only
	 * the lines that have errors in them are passed to the next steps, the other
	...
	/**
	 * Turn on safe mode during running: the transformation will run slower but with
	 ...
	 *
	 * @return Returns true if the safe mode is enabled: the transformation will run
	 *         slower but with more checking enabled
	 */
	...
	/**
	 * This adds a row producer to the transformation that just got set up. It is
	 * preferable to run this BEFORE execute() but after prepareExecution()
	 *
	 * @param stepname
	 *            The step to produce rows for
	 * @param copynr
	 *            The copynr of the step to produce row for (normally 0 unless you
	...
	/**
	 * Get the batch ID that is passed from the parent job to the transformation. If
	 * nothing is passed, it's the transformation's batch ID
	 *
	 * @return the parent job's batch ID, or the transformation's batch ID if there
	 *         is no parent job
	 */
	...
	/**
	 * Sets the passed batch ID of the transformation from the batch ID of the
	 * parent job.
	 *
	 * @param jobBatchId
	 ...
	 *
	 * @return true if the transformation is preparing for execution, false
	 *         otherwise
	 */
	 ...
	 *
	 * @param preparing
	 *            true if the transformation is preparing for execution, false
	 *            otherwise
	 */
	...
	/**
	 * Execute the transformation in a clustered fashion. The transformation steps
	 * are split and collected in a TransSplitter object
	 *
	 * @param transMeta
	...
	public static final TransSplitter executeClustered(final TransMeta transMeta,
		...
		// Pass the clustered run ID to allow for parallel execution of clustered
		// transformations
		//
		...
		try {
			...
			if (masterSteps.size() > 0) { // If there is something that needs to be done on the master...
				...
				if (executionConfiguration.isClusterPosting()) {
					...
					// Parameters override the variables but they need to pass over the
					// configuration too...
					//
					...
					String masterReply = masterServer.sendXML(transConfiguration.getXML(),
							RegisterTransServlet.CONTEXT_PATH + "/?xml=Y");
	...
	/**
	 * Monitors a clustered transformation every second, after all the
	 * transformations in a cluster schema are running. <br>
	...
	/**
	 * Cleanup the cluster, including the master and all slaves, and return the
	...
	public static int cleanupCluster(LogChannelInterface log, TransSplitter transSplitter) {
		...
		if (masterTransMeta != null && masterTransMeta.nrSteps() > 0) {
			...
			// Also de-allocate all ports used for this clustered transformation on the
			// master.
			//
			try {
	...
	public static String sendToSlaveServer(TransMeta transMeta, TransExecutionConfiguration executionConfiguration,
		...
		try {
			...
			if (executionConfiguration.isPassingExport()) {
				...
				if (!webResult.getResult().equalsIgnoreCase(WebResult.STRING_OK)) {
					throw new KettleException(
							"There was an error passing the exported transformation to the remote server: " + Const.CR
									+ webResult.getMessage());
				}
	...
	/**
	 * Checks whether the transformation is ready to start (i.e. execution
	...
	public void setInternalKettleVariables(VariableSpace var) {
		...
		// Here we don't clear the definition of the job specific parameters, as they
		// may come in handy.
		// A transformation can be called from a job and may inherit the job internal
		// variables
	...
	/**
	 * Substitutes any variable values into the given string, and returns the
	 * resolved string.
	 *
	 * @param aString
	...
	/**
	 * Substitutes any variable values into each of the given strings, and returns
	 * an array containing the resolved string(s).
	 *
	 * @param aString
	...
	/**
	 * Gets the value of the specified variable, or returns a default value if no
	 ...
	 * @param defaultValue
	 *            the default value
	 * @return the value of the specified variable, or returns a default value if no
	 *         such variable exists
	...
	/**
	 * Returns a boolean representation of the specified variable after performing
	 * any necessary substitution. Truth values include case-insensitive versions of
	 * "Y", "YES", "TRUE" or "1".
	 *
	 * @param variableName
	 *            the variable name
	 * @param defaultValue
	 *            the default value
	 * @return a boolean representation of the specified variable after performing
	 *         any necessary substitution
	 * @see org.pentaho.di.core.variables.VariableSpace#getBooleanValueOfVariable(String,
	 *      boolean)
	 */
	...
	/**
	 * Sets the values of the transformation's variables to the values from the
	 * parent variables.
	 *
	 * @param parent
	 *            the parent
	 ...
	 * @param variableValue
	 *            the variable value
	 * @see org.pentaho.di.core.variables.VariableSpace#setVariable(String,
	 *      String)
	 */
	...
	/**
	 * Shares a variable space from another variable space. This means that the
	...
	/**
	 * Injects variables using the given Map. The behavior should be that the
	 * properties object will be stored and at the time the VariableSpace is
	 * initialized (or upon calling this method if the space is already
	 * initialized). After injecting the link of the properties object should be
	 * removed.
	 *
	 * @param prop
	 *            the property map
	...
	/**
	 * Gets a list of the transformation listeners. Please do not attempt to modify
	 * this list externally. Returned list is mutable only for backward
	...
	/**
	 * Gets the list of stop-event listeners for the transformation. This is not
	 * concurrent safe. Please note this is mutable implementation only for backward
	 ...
	 * @see org.pentaho.di.core.parameters.NamedParams#addParameterDefinition(String,
	 *      String, String)
	 */
	 ...
	 *
	 * @return an array of strings containing the names of all parameters for the
	 *         transformation
	 ...
	 * @throws UnknownParamException
	 *             if the parameter does not exist
	 * @see org.pentaho.di.core.parameters.NamedParams#setParameterValue(String,
	 *      String)
	 */
	...
	/**
	 * Activates all parameters by setting their values. If no values already exist,
	 * the method will attempt to set the parameter to the default value. If no
	 * default value exists, the method will set the value of the parameter to the
	 * empty string ("").
	 *
	...
	/**
	 * Gets the parent transformation, which is null if no parent transformation
	 * exists.
	 *
	 * @return a reference to the parent transformation's Trans object, or null if
	 *         no parent transformation exists
	 */
	...
	/**
	 * Clear the error in the transformation, clear all the rows from all the row
	 * sets, to make sure the transformation can continue with other data. This is
	 * intended for use when running single threaded.
	 */
	...
	/**
	 * Sets encoding of HttpServletResponse according to System encoding.Check if
	 * system encoding is null or an empty and set it to HttpServletResponse when
	 * not and writes error to log if null. Throw IllegalArgumentException if input
	 * parameter is null.
	 *
	 * @param response
			...
			public void run() {
				try {
					...
					log.logDebug("Triggering heartbeat signal for " + getName() + " at every " + intervalInSeconds
							+ " seconds");
					ExtensionPointHandler.callExtensionPoint(log, KettleExtensionPoint.TransformationHeartbeat.id,
							Trans.this);
	...
	protected void shutdownHeartbeat(ExecutorService heartbeat) {
		...
		if (heartbeat != null) {
			...
			try {
				heartbeat.shutdownNow(); // prevents waiting tasks from starting and attempts to stop currently
											// executing ones
	...
	private int getHeartbeatIntervalInSeconds() {
		...
		// 1 - check if there's a user defined value ( transformation-specific )
		// heartbeat periodic interval;
		// 2 - check if there's a default defined value ( transformation-specific )
		// heartbeat periodic interval;
		// 3 - use default Const.HEARTBEAT_PERIODIC_INTERVAL_IN_SECS if none of the
		// above have been set
	...
	public void saveRecord(LogTableCoreInterface logTable, LogStatus status, Object subject, Object parent) {
		...
		if (logcon != null) {
			...
			try {
				...
				// Write to the standard transformation log table...
				//
				ldb.writeLogRecord(logTable, status, this, null);
			} catch (KettleDatabaseException e) {
				// PDI-9790 error write to log db is transaction error
				log.logError(BaseMessages.getString(PKG, "Database.Error.WriteLogTable", logTable), e);
				errors.incrementAndGet();
