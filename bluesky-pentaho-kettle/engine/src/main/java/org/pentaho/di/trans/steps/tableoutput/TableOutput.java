...
public class TableOutput extends BaseStep implements StepInterface {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    if ( r == null ) { // no more input to be expected...
      if (data.isHivePlugin && data.batchMode && data.hiveWritten > 0) {
        execHiveBatch();
      }
      return false;
    }
    ...
    try {
      ...
      if (data.isHivePlugin && data.batchMode) {
        outputRowData = writeToHiveTable( getInputRowMeta(), r );
      } else {
        outputRowData = writeToTable( getInputRowMeta(), r );
      }
  ...
  protected Object[] writeToTable( RowMetaInterface rowMeta, Object[] r ) throws KettleException {
    ...
    if ( insertStatement == null ) {
      if("POSTGRESQL".equals(data.databaseMeta.getPluginId())) {
        for (int i = 0; i < data.insertRowMeta.size(); i++) {
          data.insertRowMeta.getValueMeta(i).setName('\"' + data.insertRowMeta.getValueMeta(i).getName() + '\"');
        }
      }
      String sql =
        data.db
  ...
  public Object[] writeToHiveTable( RowMetaInterface rowMeta, Object[] r ) throws KettleException {
    if ( r == null ) { // Stop: last line or error encountered
      if ( log.isDetailed() ) {
        logDetailed( "Last line inserted: stop" );
      }
    ...
    if (data.hiveSql.isEmpty()) {
      data.hiveSql = data.db.getHiveInsertStatement(tableName, data.insertRowMeta);
    }
    ...
    data.hiveSql = data.db.setHiveValues(data.insertRowMeta, insertRowData, data.hiveSql, data.seg);
    data.hiveWritten++;
    data.seg = ",";
    data.batchBuffer.add( outputRowData );
    if (data.hiveWritten % data.commitSize == 0) {
      execHiveBatch();
    }
    return null;
  }
  private void execHiveBatch() throws KettleException {
    try {
      PreparedStatement insertStatement = data.db.prepareSQL(data.hiveSql);
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      try {
        ...
        if ("Hive2Shim".equals(data.databaseMeta.getPluginId())) {
          data.isHivePlugin = true;
        }
