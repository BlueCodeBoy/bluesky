...
public class KettleDatabaseRepositoryCreationHelper {
  ...
  public synchronized void createRepositorySchema( ProgressMonitorListener monitor, boolean upgrade,
    ...
    try {
      ...
      if ( dryrun ) {
        ...
        //需要调用nextVal();
        sql=addSqlForNextVal(KettleDatabaseRepository.TABLE_R_VERSION,KettleDatabaseRepository.FIELD_VERSION_ID_VERSION,sql);
      ...
      for ( int i = 0; i < code.length; i++ ) {
        ...
        if ( lookup == null ) {
          ...
          if ( dryrun ) {
            ...
            sql = addSqlForNextVal(KettleDatabaseRepository.TABLE_R_DATABASE_CONTYPE, KettleDatabaseRepository.FIELD_DATABASE_CONTYPE_ID_DATABASE_CONTYPE, sql);
            statements.add( sql );
          } else {
      ...
      for ( int i = 1; i < code.length; i++ ) {
        ...
        if ( lookup == null ) {
          ...
          if ( dryrun ) {
            ...
            sql = addSqlForNextVal(KettleDatabaseRepository.TABLE_R_LOGLEVEL,KettleDatabaseRepository.FIELD_LOGLEVEL_ID_LOGLEVEL,sql);
            statements.add( sql );
          } else {
    ...
    if ( ok_user ) {
      ...
      for ( int i = 0; i < user.length; i++ ) {
        ...
        if ( lookup == null ) {
          ...
          if ( dryrun ) {
            sql = database.getSQLOutput( null, tablename, tableData.getRowMeta(), tableData.getData(), null );
            sql = addSqlForNextVal(KettleDatabaseRepository.TABLE_R_USER, KettleDatabaseRepository.FIELD_USER_ID_USER, sql);
            statements.add( sql );
          } else {
      ...
      //
      statements.add(" CREATE TABLE T_TEST(id int); DROP TABLE T_TEST; ");
  ...
  private String addSqlForNextVal(String tableName, String pk, String sql) {
	// TODO Auto-generated method stub
	return  sql + " ;  SELECT nextval('"+tableName+"."+pk+"') ;";
}
        ...
        if ( id == null ) {
         ...
         /* if ( !create ) {
        	...
        	try {
				...
				if (message.contains("nextval") || message.contains("sefon_sequence")) {
					id = new LongObjectId(i + 1);
				}
			}
          ...
          if ( dryrun ) {
            String sql =
              database.getSQLOutput( null, KettleDatabaseRepository.TABLE_R_STEP_TYPE, table.getRowMeta(), table
                .getData(), null );
            sql = addSqlForNextVal(KettleDatabaseRepository.TABLE_R_STEP_TYPE, KettleDatabaseRepository.FIELD_STEP_TYPE_ID_STEP_TYPE, sql);
            statements.add( sql );
          } else {
        ...
        if ( id == null ) {
         ...
         /* id = new LongObjectId( i + 1 );
          ...
          if ( dryrun ) {
            ...
            sql=addSqlForNextVal(KettleDatabaseRepository.TABLE_R_DATABASE_TYPE,KettleDatabaseRepository.FIELD_DATABASE_TYPE_ID_DATABASE_TYPE,sql);
          ...
          if ( dryrun ) {
            String sql =
              database.getSQLOutput(
                ...
                null );
            sql = addSqlForNextVal(KettleDatabaseRepository.TABLE_R_JOBENTRY_TYPE,  KettleDatabaseRepository.FIELD_JOBENTRY_TYPE_ID_JOBENTRY_TYPE, sql);
            statements.add( sql );
          } else {
