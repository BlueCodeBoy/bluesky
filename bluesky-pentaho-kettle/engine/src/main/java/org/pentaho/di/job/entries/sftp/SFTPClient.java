...
public class SFTPClient {
  ...
  public void get( FileObject localFile, String remoteFile ) throws KettleJobException {
    ...
    try {
      long remoteFileLen=0;
      if(fileExists(remoteFile)){
        SftpATTRS att=c.stat(remoteFile);
        remoteFileLen=att.getSize();
      }
      localStream = KettleVFS.getOutputStream( localFile, true );
      int localFileLen=(int)localFile.getContent().getSize();
      if(remoteFileLen>localFileLen&&localFileLen>0) {
        c.get(remoteFile, localStream, null,ChannelSftp.RESUME,localFileLen);//断点续传
      }
  ...
  public void get( String localFilePath, String remoteFile ) throws KettleJobException {
    ...
    try {
      ...
      if(remoteFileLen>localFileLen&&localFileLen>0) {
        c.get(remoteFile, localFilePath, null, ChannelSftp.RESUME);//断点续传
      }
      else if(localFileLen==0){
        c.get(remoteFile, localFilePath);
      }
    } catch ( SftpException e ) {
      throw new KettleJobException( e );
    }
  ...
  public void put( FileObject fileObject, String remoteFile ) throws KettleJobException {
    ...
    try {
      ...
      long localFileLen=fileObject.getContent().getSize();
      if(localFileLen>remoteFileLen&&remoteFileLen>0) {
        c.put(inputStream, remoteFile,ChannelSftp.RESUME);//断点续传
      }
      else {
        c.put(inputStream,remoteFile);
      }
  ...
  public void put( InputStream inputStream, String remoteFile ) throws KettleJobException {
    ...
    try {
      ...
      long localFileLen=inputStream.available();
      if(localFileLen>remoteFileLen&&remoteFileLen>0) {
        c.put(inputStream, remoteFile,ChannelSftp.RESUME);//断点续传
      }
      else {
