...
/*  	  if(jobMeta.getRepositoryDirectory().getName().equals("/")) {
      ...
      if ( jobMeta.getObjectId() == null ) {
        ...
        repository.deleteJobWithTransaction(jobMeta.getObjectId(),false);
      ...
      for ( int i = 0; i < jobMeta.getSlaveServers().size(); i++ ) {
        ...
        repository.slaveServerDelegate.saveSlaveServer(slaveServer, jobMeta.getObjectId(), false, overwrite );
        //2017-11-17 by llj end
      }
      ...
      for ( int i = 0; i < jobMeta.nrJobHops(); i++ ) {
        if ( monitor != null ) {
          monitor.subTask( "保存作业连线 #" + ( i + 1 ) + "/" + jobMeta.nrJobHops() );
        }
