...
public class MergeJoinForSpark extends BaseStep implements StepInterface {
  private static Class<?> PKG = MergeJoinMetaForSpark.class; // for i18n purposes, needed by Translator2!!

  private MergeJoinMetaForSpark meta;
  private MergeJoinDataForSpark data;

  public MergeJoinForSpark(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
                           Trans trans ) {
    super( stepMeta, stepDataInterface, copyNr, transMeta, trans );
  }

  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    meta = (MergeJoinMetaForSpark) smi;
    data = (MergeJoinDataForSpark) sdi;
    int compare;
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      ...
      for (int i = 0; i < MergeJoinMetaForSpark.join_types.length; ++i ) {
        if ( joinType.equalsIgnoreCase( MergeJoinMetaForSpark.join_types[i] ) ) {
          data.one_optional = MergeJoinMetaForSpark.one_optionals[i];
          data.two_optional = MergeJoinMetaForSpark.two_optionals[i];
          return true;
        }
      }
