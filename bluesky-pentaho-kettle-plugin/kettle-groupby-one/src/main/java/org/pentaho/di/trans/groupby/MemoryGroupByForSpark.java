...
package org.pentaho.di.trans.groupby;
 ...
 * @since 2-jun-2003
 */
public class MemoryGroupByForSpark extends BaseStep implements StepInterface {
  private static Class<?> PKG = MemoryGroupByMetaForSpark.class; // for i18n purposes, needed by Translator2!!

  private MemoryGroupByMetaForSpark meta;

  private MemoryGroupByDataForSpark data;

  private boolean allNullsAreZero = false;
  private boolean minNullIsValued = false;

  public MemoryGroupByForSpark(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
                               Trans trans ) {
    super( stepMeta, stepDataInterface, copyNr, transMeta, trans );

    meta = (MemoryGroupByMetaForSpark) getStepMeta().getStepMetaInterface();
    data = (MemoryGroupByDataForSpark) stepDataInterface;
  }

  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    meta = (MemoryGroupByMetaForSpark) smi;
    data = (MemoryGroupByDataForSpark) sdi;
    ...
    if ( first ) {
      ...
      for ( int i = 0; i < meta.getSubjectField().length; i++ ) {
        if ( meta.getAggregateType()[i] == MemoryGroupByMetaForSpark.TYPE_GROUP_COUNT_ANY ) {
          data.subjectnrs[i] = 0;
        } else {
  ...
  private void handleLastOfGroup() throws KettleException {
    ...
    //
    for ( MemoryGroupByDataForSpark.HashEntry entry : data.map.keySet() ) {
      AggregateForSpark aggregate = data.map.get( entry );
    ...
    if ( data.map.isEmpty() && meta.isAlwaysGivingBackOneRow() ) {
      ...
      for ( int i = 0; i < data.aggMeta.size(); i++ ) {
        if ( meta.getAggregateType()[i] == MemoryGroupByMetaForSpark.TYPE_GROUP_COUNT_ALL
          || meta.getAggregateType()[i] == MemoryGroupByMetaForSpark.TYPE_GROUP_COUNT_ANY
          || meta.getAggregateType()[i] == MemoryGroupByMetaForSpark.TYPE_GROUP_COUNT_DISTINCT ) {
  ...
  void addToAggregate( Object[] r ) throws KettleException {
    ...
    MemoryGroupByDataForSpark.HashEntry entry = data.getHashEntry( groupData );

    AggregateForSpark aggregate = data.map.get( entry );
    if ( aggregate == null ) {
      // Create a new value...
      //
      aggregate = new AggregateForSpark();
    ...
    for ( int i = 0; i < data.subjectnrs.length; i++ ) {
      ...
      switch ( meta.getAggregateType()[i] ) {
        case MemoryGroupByMetaForSpark.TYPE_GROUP_SUM:
          aggregate.agg[i] = ValueDataUtil.sum( valueMeta, value, subjMeta, subj );
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_AVERAGE:
          if ( !subjMeta.isNull( subj ) ) {
            aggregate.agg[i] = ValueDataUtil.sum( valueMeta, value, subjMeta, subj );
            aggregate.counts[i]++;
          }
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_MEDIAN:
        case MemoryGroupByMetaForSpark.TYPE_GROUP_PERCENTILE:
          if ( !subjMeta.isNull( subj ) ) {
            ( (List<Double>) aggregate.agg[i] ).add( subjMeta.getNumber( subj ) );
          }
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_STANDARD_DEVIATION:
          ...
          aggregate.agg[i] = sum;
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_COUNT_DISTINCT:
          ...
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_COUNT_ALL:
          if ( !subjMeta.isNull( subj ) ) {
            aggregate.counts[i]++;
          }
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_COUNT_ANY:
          aggregate.counts[i]++;
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_MIN:
          ...
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_MAX:
          if ( subjMeta.compare( subj, valueMeta, value ) > 0 ) {
            aggregate.agg[i] = subj;
          }
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_FIRST:
          if ( !subjMeta.isNull( subj ) && value == null ) {
            aggregate.agg[i] = subj;
          }
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_LAST:
          if ( !subjMeta.isNull( subj ) ) {
            aggregate.agg[i] = subj;
          }
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_FIRST_INCL_NULL:
          if ( aggregate.counts[i] == 0 ) {
            aggregate.agg[i] = subj;
            aggregate.counts[i]++;
          }
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_LAST_INCL_NULL:
          aggregate.agg[i] = subj;
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_CONCAT_COMMA:
          if ( !( subj == null ) ) {
          ...
          break;
        case MemoryGroupByMetaForSpark.TYPE_GROUP_CONCAT_STRING:
          if ( !( subj == null ) ) {
            String separator = "";
   ...
   * @throws KettleException
   */
  void newAggregate( Object[] r, AggregateForSpark aggregate ) throws KettleException {
    if ( aggregate == null ) {
   ...
   */
  Object[] getAggregateResult( AggregateForSpark aggregate ) throws KettleValueException {
      ...
      for ( int i = 0; i < data.subjectnrs.length; i++ ) {
        ...
        switch ( meta.getAggregateType()[i] ) {
          ...
          case MemoryGroupByMetaForSpark.TYPE_GROUP_PERCENTILE:
            ...
            if ( meta.getAggregateType()[i] == MemoryGroupByMetaForSpark.TYPE_GROUP_PERCENTILE ) {
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      data.map = new HashMap<MemoryGroupByDataForSpark.HashEntry, AggregateForSpark>( 5000 );
  ...
  public void dispose( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    ( (MemoryGroupByDataForSpark) sdi ).clear();
  }
